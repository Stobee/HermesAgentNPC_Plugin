...
- **스펙상 불가능한 입력은 조용히 무시하지 않는다.** 프로토콜이 보장한다고 적힌 것이 어긋난 채
  도착하면 반드시 `Warning` 이상으로 남긴다. 이 로그를 읽는 사람은 **Hermes 서버를 만드는 우리
  자신**이다. 조용히 넘기면 증상이 "서버가 틀렸다"가 아니라 "게임이 이상하다"로 나타나 엉뚱한
  곳을 파게 된다. 해당하는 지점:
...
> **2026-07-29 추가 요구:** 프로토콜 §3.6이 턴 직렬화를 확정했으므로 누적 버퍼는 하나면
> 충분하다. 다만 **델타의 `id`가 진행 중인 턴과 달라지면 버퍼를 합치지 말고 버리고 새로
> 시작한다.** 서버가 규약을 어기고 두 턴을 교차 전송했을 때 두 답변이 뒤섞인 문자열이
> 화면에 남는 것을 막는 방어다(프로토콜 §4.9, §7 4항).
